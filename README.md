...
- Stereo Width, Exciter, DeepBass, Dynamic Auto Volume, LUFS-нормализация станций, GainRider и финальный лимитер;
...
1. Скачайте support-архив `IRPFFmpeg-v1.0.1-support-win-x64.zip` из релиза `v1.0.1`, если у вас еще нет рабочей папки с DLL и служебными файлами.
2. Распакуйте его в отдельную папку.
3. Скачайте актуальный `IRPFFmpeg.exe` из последнего релиза, например `v1.0.2`, и положите рядом с `Start_IRPFFmpeg.exe`.
4. Запускайте `Start_IRPFFmpeg.exe`.

Support-архив нужен для библиотек, загрузчика, плейлиста, настроек и файлов языка. При небольших исправлениях обычно достаточно заменить только `IRPFFmpeg.exe`.

Минимальный состав рабочей папки после распаковки support-архива и добавления `IRPFFmpeg.exe`:
...
Если у вас уже есть рабочая папка с `heap_dll`, `Language`, `playlist.m3u` и `app.dat`, для обновления чаще всего достаточно скачать новый standalone `IRPFFmpeg.exe` и заменить старый файл.

...
2. Скачайте project-архив актуальной версии, например `IRPFFmpeg-v1.0.3-project-win-x64.zip`.
...
Версионные project-архивы содержат только файлы IRPFFmpeg: `IRPFFmpeg.exe`, `Start_IRPFFmpeg.exe`, файлы языка, документацию и служебные файлы проекта. Начиная с последующих релизов, например `v1.0.4`, новые версии публикуются без сторонних DLL; достаточно распаковать новый project-архив поверх рабочей папки с уже установленным `heap_dll`.

При небольших исправлениях, которые меняют только основное приложение, обычно достаточно заменить только standalone `IRPFFmpeg.exe`.
...
Если у вас уже есть рабочая папка с `heap_dll`, для обновления чаще всего достаточно скачать новый project-архив или standalone `IRPFFmpeg.exe`. Если в релизе менялись `Start_IRPFFmpeg.exe`, `Language/*.lng`, документация или служебные файлы проекта, используйте project-архив, а не только exe.

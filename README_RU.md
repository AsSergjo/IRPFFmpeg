...
[Короткое видео с работой IRPFFmpeg на YouTube](https://youtu.be/AuAjhOTRdEU).

![Обычный режим IRPFFmpeg](normal_mode.png)

Главное окно: управление плейлистом, кнопки воспроизведения, статус потока, обложка, история и визуализация спектра в обычном режиме.

![Параметрический эквалайзер IRPFFmpeg](equalizer_window.png)

Параметрический эквалайзер: шесть полос с независимой настройкой Gain/Q и живым графиком результирующей АЧХ.

![Компактный режим IRPFFmpeg](compact_mode.png)

Компактный режим: небольшое окно спектра со строкой текущего трека, кнопкой предыдущей станции и кнопкой возврата в обычный режим.

![Компактный режим IRPFFmpeg без спектрограммы](compact_mode_without_spektr.png)

Компактный режим без спектрограммы: время воспроизведения, текущий трек и кнопки в одной узкой строке.

![Окно настроек IRPFFmpeg](window_programm_setting_rus.png)

Окно настроек: выбор языка, аудиообработка, поведение в трее, показ обложки при смене трека и параметры компактного режима.
...
- сохранение заданного пользователем названия станции при обновлении метаданных потока;
...
- регулировка громкости отдельным дуговым контролом и настройка шестиполосного параметрического эквалайзера с живым графиком суммарной АЧХ;
- Stereo Width, Exciter, DeepBass, компрессор разборчивости речи, LUFS-нормализация станций, GainRider и финальный лимитер;
- сворачивание в системный трей;
- компактный режим с визуализацией спектра, временем текущего сеанса, строкой трека, кнопкой возврата к предыдущей станции и кнопкой возврата в обычный режим;
- возможность держать компактное окно поверх всех окон;
- режим без спектрограммы, в котором компактное окно сокращается до строки текущего трека и кнопок;
- сохранение позиции компактного окна между переключениями режима и перезапусками программы;
...
2. Скачайте project-архив актуальной версии, например `IRPFFmpeg-v1.2.4-project-win-x64.zip`.
